...
export default function ProductDetailWorkflow() {
  ...
  const generatePersonalizationPrompt = (product: string, device: DeviceType): string => {
    const size = SIZE_CONFIG[device];
    const style = getCurrentColorStyle();
    const personalizationPlacement = "MANDATORY PHYSICS COMPLIANCE: the blanket is draped over a chair or furniture with natural gravity-driven folds and sagging, the printed design is MOSTLY OBSCURED (only 30-40% visible) with the MAJORITY HIDDEN by draping, chair edges, and fabric folds, the pattern is SEVERELY INTERRUPTED where the fabric bends over the chair arms and seat, the bottom portion of the design is naturally BUNCHING and FOLDING due to gravity, it is PHYSICALLY IMPOSSIBLE to see the complete design when draped over furniture, the focus is on the overall aesthetic not the full pattern visibility";
    return `Amazon A+ Personalization module ${size.label}, ${product} showing the personalized custom design result with simple customization steps, ${personalizationPlacement}, the blanket displays the custom photo/name/message printed on it draped naturally over furniture, MUST INCLUDE clear step-by-step customization guide: "Step 1: Upload Your Photo", "Step 2: Add Name & School", "Step 3: We Create Your Gift" with elegant icons, romantic/emotional headline like "Personalized Just for You" or "A Gift Made for [Name]", elegant script typography, warm sentimental atmosphere, ${style.colorPalette}, ${style.lightingStyle}, emotional gift photography emphasizing the personal connection`;
  };
